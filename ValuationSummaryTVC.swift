...
//            if indexPath.row == 0 {
//                
//                value = share.analysis?.adjFutureGrowthRate.valuesOnly(dateOrdered: .ascending)?.mean()
//                if value == nil {
//                    value = share.analysis?.future_growthNextYear.valuesOnly(dateOrdered: .ascending)?.last
//                }
...
//                
//                let da = [share.ratios?.bvps.datedValues(dateOrder: .ascending)?.dropZeros() ?? [], share.income_statement?.eps_annual.datedValues(dateOrder: .ascending)?.dropZeros() ?? []]
//                
//                let harmonisedArrays = ValuationDataCleaner.harmonizeDatedValues(arrays: da)
//                                
//                if let futureGrowth = share.rule1Valuation?.r1CalculatedFutureGrowth(bvps: share.ratios?.bvps.datedValues(dateOrder: .ascending)?.dropZeros()) {
