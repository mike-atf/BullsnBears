...
// 4 slower download tasks for more data do on a background thread, using NSManagedObjectID
        ...
        Task.init() {
//            do {
//                try await YahooPageScraper.keyratioDownloadAndSave(shareSymbol: symbol, shortName: newShare.name_short, shareID: newShareID)
                await YahooPageScraper.dataDownloadAnalyseSave(symbol: symbol!,shortName: shortName!,shareID: newShareID, option: .yahooKeyStatistics, downloadRedirectDelegate: controller)
                await controller.getDataForNewShare(objectID: newShareID, url: url, symbol: symbol)
