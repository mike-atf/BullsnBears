...
class StocksController2: NSFetchedResultsController<Share> {
    ...
    func getDataForNewShare(objectID: NSManagedObjectID, url: URL?, symbol: String?) async throws {
        ...
        await YahooPageScraper.dataDownloadAnalyseSave(symbol: newShare.symbol!, shortName: newShare.name_short!, shareID: objectID, option: .allPossible, progressDelegate: viewController, downloadRedirectDelegate: self)
                    ...
                    if let dDivs = await datedDividends {
                        ...
                        if let latestClose = share.latestPrice(option: .close)  {
                            ...
                            if let valid = price3yAgo?.close {
                                ...
                                ErrorController.addInternalError(errorLocation: #function, errorInfo: "can't find 3 year closing price from \(price3yAgo) to work out 3-year-return")
                            }
                            ...
                            if let valid = price10yAgo?.close {
                                ...
                                ErrorController.addInternalError(errorLocation: #function, errorInfo: "can't find 10 year closing price  \(price10yAgo) to work out 3-year-return")
                            }
                        }
                        else {
...
//            let latestQEPSDate = share.wbValuation?.epsQWithDates()?.last?.date
            let latestQEPSDate = share.income_statement?.eps_quarter.datedValues(dateOrder: .ascending)?.dropZeros().last?.date
            ...
            Task.init(priority: .background, operation: {
               ...
               await backgroundMoc?.perform({
                    ...
                    do {
                        ...
                        let income = backgroundShare.income_statement ?? Income_statement(context: self.backgroundMoc!)
                        income.share = backgroundShare
...
//                            let ldv = Labelled_DatedValues(label: "Quarterly EPS", datedValues: valid)
//                            let income = backgroundShare.income_statement ?? Income_statement(context: self.backgroundMoc!)
//                            income.share = backgroundShare

                            if let existingDVs = income.eps_annual.datedValues(dateOrder: .ascending) {
                                income.eps_annual = existingDVs.mergeIn(newDV: valid)?.convertToData()
                            } else {
                                income.eps_annual = valid.convertToData()
                            }
...
//                            backgroundShare.wbValuation?.saveEPSTTMWithDateArray(datesValuesArray: valid, saveToMOC: false)
                        ...
                        ErrorController.addInternalError(errorLocation: "StocksController2.updateStocksData", systemError: error, errorInfo: "error fetching from and/or saving backgroundMOC during 'updateStocksData' function ")
                    }
