...
//                }
            ...
            // Revenue
                ...
                if let revenueDVs = share.income_statement?.revenue.datedValues(dateOrder: .ascending, oneForEachYear: true)?.dropZeros() {
                    ...
                    if let growthrates = Calculator.ratesOfGrowth(datedValues: revenueDVs) {
...
//                }
            ...
            // net income
                ...
                if let datedValues = share.income_statement?.netIncome.datedValues(dateOrder: .ascending, oneForEachYear: true)?.dropZeros() {
                    ...
                    if let growthrates = Calculator.ratesOfGrowth(datedValues: datedValues) {
                        if let growthEMA = growthrates.ema(periods: emaPeriod) {
                            value$ = percentFormatter0DigitsPositive.string(from: growthEMA as NSNumber) ?? "-"
                            color = GradientColorFinder.gradientColor(lowerIsGreen: false, min: 0, max: 100, value: growthEMA, greenCutoff: 0, redCutOff: 0)
...
//                proportions = Calculator.compoundGrowthRates(values: array1)
    ...
    func downloadAllValuationData() {
        ...
        downloadTask = Task.init(priority: .background) {
            ...
            do {
                progressDelegate?.allTasks = MacrotrendsScraper.mtAnnualDataRowTitles(options: .allPossible).count + 2 + YahooPageScraper.yahooAllDataRowTitles().compactMap{ $0 }.count
...
//                    try await YahooPageScraper.keyratioDownloadAndSave(shareSymbol: symbol, shortName: shortName, shareID: shareID)
                    await YahooPageScraper.dataDownloadAnalyseSave(symbol: symbol, shortName: shortName, shareID: shareID, option: .yahooKeyStatistics, progressDelegate: progressDelegate, downloadRedirectDelegate: self)
...
//                    try await MacrotrendsScraper.wbvDataDownloadAnalyseSave(shareSymbol: symbol,  shortName: shortName, shareID: shareID,downloadRedirectDelegate: self)
                    await MacrotrendsScraper.dataDownloadAnalyseSave(shareSymbol: symbol, shortName: shortName, shareID: shareID, downloadOption: .wbvOnly, downloadRedirectDelegate: self)
                }
                // US stocks
                else {

                    await MacrotrendsScraper.dataDownloadAnalyseSave(shareSymbol: symbol, shortName: shortName, shareID: shareID, downloadOption: .allPossible, progressDelegate: progressDelegate, downloadRedirectDelegate: self)
...
//                }
                ...
                await Rule1Valuation.downloadAnalyseAndSave(shareSymbol: symbol, shortName: shortName!, shareID: shareID, progressDelegate: nil, downloadRedirectDelegate: self)
