...
//        }
    ...
    class func downloadAnalyseAndSave(shareSymbol: String?, shortName: String, shareID: NSManagedObjectID, progressDelegate: ProgressViewDelegate?=nil, downloadRedirectDelegate: DownloadRedirectionDelegate) async {
...
//                async let yahooLVS = YahooPageScraper.rule1DownloadAndAnalyse(symbol: symbol!, shareID: shareID, downloadRedirectDelegate: nil)?.sortAllElementDatedValues(dateOrder: .ascending)
                ...
                await YahooPageScraper.dataDownloadAnalyseSave(symbol: valid, shortName: shortName! ,shareID: shareID, option: .rule1Only, downloadRedirectDelegate: nil)
