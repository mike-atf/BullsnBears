...
@objc(Rule1Valuation)
public class Rule1Valuation: NSManagedObject, Codable {
...
//        self.share?.moatCategory = "NA"
        ...
        for i in 0..<moatArrays.count {
            ...
            if let returnRates = Calculator.reatesOfReturn(datedValues: moatArrays[i]) {
        ...
        if sumValidRates < 20 {
            errors = ["Only \(sumValidRates)/50 Big Five moat parameters available. Moat score is not reliable"]
            return (errors, nil)
...
//    }
    ...
    func r1CalculatedFutureGrowth(bvps: [DatedValue]?) -> Double? {
        
        if let valid = share?.analysis?.adjFutureGrowthRate.valuesOnly(dateOrdered: .ascending,withoutZeroes: true)?.mean() {
            return valid
        }
        else if let valid = share?.analysis?.future_revenueGrowthRate.valuesOnly(dateOrdered: .ascending,withoutZeroes: true)?.mean() {
            return valid
        }
        else {
            return bvps?.growthRates(dateOrder: .ascending)?.values().mean()
        }
    ...
    func futurePER(futureGrowth: Double) -> Double? {
        ...
        if let ratios = share?.ratios {
            hxPE = ratios.pe_ratios.valuesOnly(dateOrdered: Order.descending, withoutZeroes: true)
        }
    ...
    func stickerPrice() -> (Double?, [String]?) {
        ...
        let bvps = share?.ratios?.bvps.datedValues(dateOrder: .ascending)?.dropZeros()
        let eps = share?.income_statement?.eps_annual.valuesOnly(dateOrdered: .descending, withoutZeroes: true)
...
//        let cleanedBVPS = bvps!
        ...
        guard bvps?.count ?? 0 > 1 else {
            errors.append("no book value per share figure available.")
            return (nil, errors)
        }
        
        guard let futureGrowth = r1CalculatedFutureGrowth(bvps: bvps) else {
